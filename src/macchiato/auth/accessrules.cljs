(ns macchiato.auth.accessrules
  "Access Rules system for ring based applications."
  (:require [macchiato.auth :refer [unauthorized]]
...
(defn handle-error
  ...
  {:no-doc true}
  [response request respond raise {:keys [reject-handler on-error redirect]}]
  ...
  (let [val (get-value response)
    ...
    (cond
      (string? redirect)
      (respond (r/found redirect))

      (fn? on-error)
      (respond (on-error request val))
...
(defn wrap-access-rules
  ...
  (when (nil? rules)
    (throw (js/Error "rules should not be empty.")))
      ...
      (if-let [match (match-access-rules accessrules request)]
        (let [res (apply-matched-access-rule match request)]
          (if (success? res)
            ...
            (handle-error res request respond raise (merge opts match))
            ))
        (case policy
          :allow (handler request respond raise)
          :reject (handle-error (error nil) request respond raise opts))))))
  ...
  (let [match (compile-access-rule rule)]
    (fn [request respond raise]
      (let [rsp (apply-matched-access-rule match request)]
        (if (success? rsp)
          (handler request)
          (handle-error rsp request respond raise rule))))))